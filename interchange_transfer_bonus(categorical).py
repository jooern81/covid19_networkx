...
start_date = datetime.date(2019,1,19)
end_date = datetime.date(2019,1,26)
delta = datetime.timedelta(days=1)
list_of_dates = []
list_of_hours = range(7,24)
list_of_adults = []
list_of_children = []
list_of_seniors = []
...
for date in list_of_dates:
    for hour in list_of_hours:
        try:
            interchange_transfer_bonus_dict = get_interchange_transfer_bonus(adult_df, sd, date, hour)
            list_of_adults.append(generate_report(interchange_transfer_bonus_dict,date,hour))
            interchange_transfer_bonus_dict = get_interchange_transfer_bonus(child_df, sd, date, hour)
            list_of_children.append(generate_report(interchange_transfer_bonus_dict,date,hour))
            interchange_transfer_bonus_dict = get_interchange_transfer_bonus(senior_df, sd, date, hour)
            list_of_seniors.append(generate_report(interchange_transfer_bonus_dict,date,hour))
        except:
            print(str(date)+str(hour)+' not available.')

df_adults = pd.concat(list_of_adults)
df_adults.to_excel("interchange_transfer_bonus(adults).xlsx") 
df_children = pd.concat(list_of_children)
df_children.to_excel("interchange_transfer_bonus(children).xlsx") 
df_seniors = pd.concat(list_of_seniors)
df_seniors.to_excel("interchange_transfer_bonus(seniors).xlsx") 